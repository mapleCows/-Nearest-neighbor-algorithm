...
void PrintVector(vector<double>);				//basic printing vector helper
void ParseData(vector<Data>&, string);			//parse txt file
double CalcAccuracyStub(vector<double>);		//stub with rand for testing
void forwardSelection(vector<Data>&);			//foward
void backwardSelection(vector<Data>&);			//backward
double Accuracy(vector<Data>& v, int currFeatures, vector<int> bestFeatures);		//calculating accuracy of a set
double Distance(vector<double> A1, vector<double> B2, vector<int> bestfeatures);	//equation calculation with summation for num of features 
void SearchAndDestroy(vector<int>&, int);											//removing a feature from the set for backwards selection
...
void ParseData(vector<Data>& v, string file)
{
	...
	while (getline(dataFile, line)) {
		...
		while (iss >> d) {												//parsing file line by line
			temp = atof(d.c_str());
			attr.push_back(temp);
...
double CalcAccuracy(vector<double>) //stub
{

	return rand();
}

void forwardSelection(vector<Data>& v)
{
	...
	for (int i = 1; i < v.at(1).attributes.size(); ++i) {			//pushing back feature list
	...
	for (int i = 1; i < v.at(1).attributes.size(); ++i) {
		...
		//keeping track of local max, set max and run max
		if (i == 1) {
...
void backwardSelection(vector<Data>&v)
{
	vector<int> currFeatures;
	vector<int> bestFeatures;
	int featureToAdd = 0;
	...
	for (int i = 1; i < v.at(1).attributes.size(); ++i) {
		...
		for (int k = 0; k < currFeatures.size(); ++k) {	//running throught all features finding best accuracy
			...
			//keeping track of lowest accuracy of current i level
			if (accuracy < bestAccuracy) {
				bestAccuracy = accuracy;
				featureToAdd = currFeatures.at(k);
				if (MAXAccuracy < bestAccuracy) {
		...
		SearchAndDestroy(bestFeatures, featureToAdd);				//removing worst feature
		for (int p = 0; p < currFeatures.size(); ++p) {		//removing best feature from consideration pool
			if (currFeatures.at(p) == featureToAdd) {
				currFeatures.erase(currFeatures.erase(currFeatures.begin() + p, currFeatures.begin() + p));
			}
		...
		cout << "\nOn level " << i << " I removed feature " << featureToAdd << " to the best features set\n\n";
...
double Accuracy(vector<Data>& v, int currFeatures, vector<int> bestFeatures)
{
	...
	for (int i = 0; i < v.size(); ++i) {
		...
		for (int k = 0; k < v.size(); ++k) {
			...
			if (currDistance < minDistance) {										//keeping track of who is the neigbor
				minDistance = currDistance;
				neigborIndex = k;
			}
		}
		if (v.at(i).attributes.at(0) == v.at(neigborIndex).attributes.at(0)) { ++numCorrect; }	//couting num correct

	}
	double p = v.size();	//calc accuracy 
	double n = numCorrect;
	double b = n / p;
	return b * 100;
}
...
void SearchAndDestroy(vector<int>& v, int index)
{
	for (int i = 0; i < v.size(); ++i) {
